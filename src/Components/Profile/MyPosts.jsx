            ...
            {posts?.map((post) => <div className='border-bottom text-start p-3' key={post?._id}>
                <div className='d-flex justify-content-between'>
                    ...
                    {(post?.authorId?._id !== userId && post?.authorId !== userId) ?
                        ...
                        <button className="add_friend_btn" onClick={() => addFirend(user._id, post._id)}>
                            {user?.firendRequest?.includes(userId) ? "pendding" : <span>Add Friend<i className="fa-solid fa-user-plus mx-2"></i></span>}
                        </button> :
                        <h6>
